#------------------------------------------------------
#        SIMPLE TURBOFAN CYCLE PERFORMANCE MODEL
#------------------------------------------------------
import time


start_time  = time.time()

exec( open( "pop.std").read())

loadDirectory( "../pop" ) 
loadDirectory( "../Brayton" ) 

exec( open( "temp.include").read())
import varsg
varsg.out = open( "pop.out", "a")
varsg.pretty = open( "pretty.out", "a")

#---------------------------------------------------------------------------
# create all of the component objects, including the shaft connection ports
# --------------------------------------------------------------------------
start = FlightConditionsSMJ( "start" )
inlet = Duct( "inlet" )
fan = Compressor( "fan" )
splitter = Splitter( "splitter" )
duct2 = Duct( "duct2" )
LPC = Compressor( "LPC" )
duct25 = Duct( "duct25" )
HPC = Compressor( "HPC" )
burner = Burner( "burner" )
HPT = Turbine( "HPT" )
duct45 = Duct( "duct45" )
LPT = Turbine( "LPT" )
duct5 = Duct( "duct5" )
priNozzle = Nozzle( "priNozzle" )

duct17 = Duct( "duct17" )
fanNozzle = Nozzle( "fanNozzle" )
HPshaft = Shaft( "HPshaft" )
LPshaft = Shaft( "LPshaft" )

LPshaft.MPfan = MP( LPshaft, io="in" )
LPshaft.MPlpc = MP( LPshaft, io="in" )
LPshaft.MPlpt = MP( LPshaft, io="in" )

HPshaft.MPC = MP( HPshaft, io="in" )
HPshaft.MPT = MP( HPshaft, io="in" )

Perf = Perf( "Perf" )

#--------------------------------------
# link the objects together
# ------------------------------------- 
inlet.FNi.linkFN( start.FNo )
fan.FNi.linkFN( inlet.FNo )
splitter.FNi.linkFN( fan.FNo )
duct17.FNi.linkFN( splitter.FNo2 )
fanNozzle.FNi.linkFN( duct17.FNo )
duct2.FNi.linkFN( splitter.FNo1 )
LPC.FNi.linkFN( duct2.FNo )
duct25.FNi.linkFN( LPC.FNo )
HPC.FNi.linkFN( duct25.FNo )
burner.FNi.linkFN( HPC.FNo )
HPT.FNi.linkFN( burner.FNo )
duct45.FNi.linkFN( HPT.FNo )
LPT.FNi.linkFN( duct45.FNo )
duct5.FNi.linkFN( LPT.FNo )
priNozzle.FNi.linkFN( duct5.FNo )

LPshaft.MPfan.linkMP( fan.MP )
LPshaft.MPlpc.linkMP( LPC.MP )
LPshaft.MPlpt.linkMP( LPT.MP )

HPshaft.MPC.linkMP( HPC.MP )
HPshaft.MPT.linkMP( HPT.MP )

HPT.FNiBld1.linkFN( HPC.FNoBld2 )
HPT.FNiBld2.linkFN( HPC.FNoBld1 )


#------------------------------------------------------------------------
# set component variable values to match N+3 reference cycle at SLS
#------------------------------------------------------------------------ 
start.alt.set( 35000. )
start.MN.set( 0.85 )
start.W.set( 815.49 )

# use cantera for fluid properties
#start.comp.set( "canteraFN" )


# use tables for fluid properties
start.comp.set( "newtherm" ) 

inlet.dP.set( 0.002 )  # inlet recovery 0.998
#inlet.FNo.MN.set( 0.625 )

fan.PRdes.set( 1.300 )
fan.effDes.set( 0.9689 )
fan.NcMapDes.set( 1.000 )
fan.RlineDes.set( 2.000 ) 
fan.NcMapDes.set( 1.00 )
fan.RlineDes.set( 2.00 ) 
#fan.FNo.MN.set( 0.45 )

splitter.BPR.set( 23.8470 )
splitter.BPR.set( 17. )
#splitter.FNo1.MN.set( 0.45 )
#splitter.FNo2.MN.set( 0.45 )

duct2.dP.set( 0.010 )
#duct2.FNo.MN.set( 0.45 )

LPC.PRdes.set( 3.000 )
LPC.effDes.set( 0.8895 )
LPC.NcMapDes.set( 1.00 )
LPC.RlineDes.set( 2.00 ) 
LPC.Wfrac1.set( 1e-8 )
LPC.hfract1.set( 0.7 )
LPC.Wfrac2.set( 1e-8 )
LPC.hfract2.set( 0.9 )
#LPC.FNo.MN.set( 0.45 )

duct25.dP.set( 0.015 )
#duct25.FNo.MN.set( 0.45 )

HPC.PRdes.set( 14.1030 )
HPC.effDes.set( 0.8469 )
HPC.NcMapDes.set( 1.00 )
HPC.RlineDes.set( 2.00 ) 
HPC.Wfrac1.set( 0.07582 )
HPC.hfract1.set( 0.370 )
HPC.Wfrac2.set( 0.07582 )
HPC.hfract2.set( 0.386 )
#HPC.FNo.MN.set( 0.30 )

burner.FAR.set( 0.02780 )
burner.LHV.set( -1830. )
burner.dP.set( 0.0400 )
#burner.FNo.MN.set( 0.10 )

HPT.PRmapDes.set( 3.0 )
HPT.PR.set( 3.5 )
HPT.effDes.v = 0.9225
HPT.NcMapDes.set( 0.9 )
#HPT.FNo.MN.set( 0.30 )

duct45.dP.set( 0.005 )
#duct45.FNo.MN.set( 0.45 )

LPT.PRmapDes.set( 6.0 )
LPT.PR.set( 2. )
LPT.effDes.v = 0.9400
LPT.NcMapDes.set( 0.9 )
#LPT.FNo.MN.set( 0.35 )

duct5.dP.set( 0.010 )
#duct5.FNo.MN.set( 0.25 )

priNozzle.PsExh.set( "start.Pamb" )

duct17.dP.set( 0.015 )
#duct17.FNo.MN.set( 0.45 )

fanNozzle.PsExh.set( "start.Pamb" )

HPshaft.N.set( 10000. )
HPshaft.I.set( 6.)
HPshaft.HPX.set( 150. )

LPshaft.N.set( 10000. )
LPshaft.I.set( 6.)



# =======================================================================
#                           E3 fan tip map
# =======================================================================
# 16 speeds
# 10 index (stall margins)
fan.WcTable.x = [ 0.20, 0.30, 0.40, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00, 1.05, 1.10 ]
fan.WcTable.y = [ 1.00, 1.20, 1.40, 1.60, 1.80, 2.00, 2.20, 2.40, 2.60, 2.80 ] 
fan.WcTable.data = [
[  224.751,  231.480,  238.209,  244.908,  251.637,  258.337,  265.066,  271.765,  278.435,  281.710 ],
[  342.216,  352.280,  362.373,  372.437,  382.471,  392.565,  402.569,  412.603,  420.925,  430.929 ],
[  460.717,  474.146,  487.574,  501.002,  514.371,  526.087,  539.426,  551.112,  562.769,  572.684 ],
[  567.622,  586.126,  604.601,  623.076,  641.432,  659.788,  674.779,  693.076,  706.296,  716.091 ],
[  615.260,  637.129,  658.999,  680.808,  702.529,  724.219,  742.516,  760.723,  772.171,  781.878 ],
[  678.128,  703.362,  728.566,  753.740,  778.825,  803.820,  822.057,  836.781,  849.822,  857.787 ],
[  752.861,  781.460,  810.028,  838.537,  866.897,  895.108,  911.484,  922.813,  930.719,  936.942 ],
[  770.100,  810.623,  851.116,  891.520,  931.804,  968.486,  994.807, 1010.915, 1018.612, 1022.945 ],
[  802.629,  851.652,  900.675,  949.609,  998.424, 1045.318, 1081.643, 1097.603, 1103.468, 1105.970 ],
[  850.505,  906.317,  962.158, 1017.881, 1073.425, 1127.018, 1164.877, 1177.353, 1179.735, 1178.753 ],
[  894.988,  962.754, 1030.460, 1098.047, 1165.397, 1232.151, 1256.283, 1263.504, 1262.314, 1262.805 ],
[  968.902, 1045.079, 1121.197, 1197.136, 1272.747, 1330.733, 1346.216, 1351.606, 1351.949, 1350.610 ],
[ 1129.771, 1192.192, 1254.465, 1316.530, 1371.359, 1390.505, 1396.132, 1398.396, 1399.006, 1397.905 ],
[ 1258.261, 1311.944, 1363.706, 1404.988, 1425.965, 1431.742, 1434.154, 1434.883, 1435.613, 1434.661 ],
[ 1337.668, 1384.235, 1420.500, 1441.625, 1455.902, 1461.768, 1464.270, 1463.407, 1464.226, 1463.363 ],
[ 1397.856, 1432.439, 1460.114, 1476.014, 1485.185, 1490.991, 1491.751, 1490.828, 1489.906, 1490.665 ],
]

fan.effTable.x = [ 0.20, 0.30, 0.40, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00, 1.05, 1.10 ]
fan.effTable.y = [ 1.00, 1.20, 1.40, 1.60, 1.80, 2.00, 2.20, 2.40, 2.60, 2.80 ] 
fan.effTable.data = [
[ 0.6800, 0.6800, 0.6800, 0.6800, 0.6840, 0.6860, 0.6850, 0.6800, 0.6800, 0.6800 ],
[ 0.7230, 0.7150, 0.7480, 0.7430, 0.7500, 0.7500, 0.7500, 0.7070, 0.6840, 0.6800 ],
[ 0.7520, 0.7410, 0.7630, 0.8000, 0.8000, 0.7930, 0.7630, 0.7520, 0.7170, 0.6800 ],
[ 0.7630, 0.7790, 0.8110, 0.8240, 0.8340, 0.8210, 0.7960, 0.7440, 0.6990, 0.6800 ],
[ 0.7500, 0.7880, 0.8160, 0.8400, 0.8400, 0.8400, 0.8070, 0.7620, 0.6800, 0.6800 ],
[ 0.7070, 0.7780, 0.8200, 0.8420, 0.8540, 0.8460, 0.8070, 0.7490, 0.6800, 0.6800 ],
[ 0.7460, 0.7990, 0.8430, 0.8600, 0.8600, 0.8410, 0.7650, 0.7030, 0.6800, 0.6800 ],
[ 0.7010, 0.7520, 0.8090, 0.8430, 0.8720, 0.8740, 0.8300, 0.7210, 0.6800, 0.6800 ],
[ 0.7000, 0.7330, 0.7750, 0.8360, 0.8700, 0.8900, 0.8590, 0.7750, 0.6800, 0.6800 ],
[ 0.7000, 0.7120, 0.7770, 0.8370, 0.8750, 0.9000, 0.8610, 0.7770, 0.6800, 0.6800 ],
[ 0.7000, 0.7110, 0.7840, 0.8220, 0.8860, 0.9000, 0.8300, 0.7290, 0.6800, 0.6800 ],
[ 0.7030, 0.7450, 0.7930, 0.8500, 0.8940, 0.8950, 0.8230, 0.7150, 0.6800, 0.6800 ],
[ 0.7660, 0.8120, 0.8380, 0.8780, 0.8910, 0.8750, 0.8150, 0.7570, 0.6800, 0.6800 ],
[ 0.8170, 0.8400, 0.8630, 0.8800, 0.8760, 0.8480, 0.8260, 0.7790, 0.7230, 0.6800 ],
[ 0.8200, 0.8420, 0.8600, 0.8600, 0.8590, 0.8440, 0.8370, 0.8090, 0.7710, 0.7300 ],
[ 0.6800, 0.7440, 0.7940, 0.7360, 0.8010, 0.7100, 0.7570, 0.6800, 0.7080, 0.7090 ],
]

fan.PRtable.x = [ 0.20, 0.30, 0.40, 0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00, 1.05, 1.10 ]
fan.PRtable.y = [ 1.00, 1.20, 1.40, 1.60, 1.80, 2.00, 2.20, 2.40, 2.60, 2.80 ] 
fan.PRtable.data = [
[ 1.020, 1.020, 1.020, 1.018, 1.018, 1.016, 1.016, 1.014, 1.011, 1.006 ],
[ 1.049, 1.047, 1.047, 1.045, 1.042, 1.042, 1.037, 1.033, 1.028, 1.023 ],
[ 1.090, 1.088, 1.086, 1.084, 1.079, 1.075, 1.068, 1.063, 1.056, 1.046 ],
[ 1.140, 1.140, 1.138, 1.136, 1.127, 1.119, 1.110, 1.098, 1.084, 1.067 ],
[ 1.170, 1.170, 1.169, 1.166, 1.157, 1.146, 1.134, 1.117, 1.098, 1.076 ],
[ 1.204, 1.204, 1.202, 1.198, 1.189, 1.175, 1.160, 1.136, 1.112, 1.087 ],
[ 1.244, 1.243, 1.241, 1.236, 1.222, 1.199, 1.174, 1.148, 1.120, 1.091 ],
[ 1.267, 1.275, 1.281, 1.282, 1.276, 1.257, 1.223, 1.182, 1.142, 1.101 ],
[ 1.298, 1.311, 1.324, 1.332, 1.333, 1.320, 1.281, 1.232, 1.183, 1.134 ],
[ 1.341, 1.357, 1.375, 1.386, 1.387, 1.373, 1.325, 1.269, 1.214, 1.158 ],
[ 1.383, 1.409, 1.432, 1.447, 1.450, 1.418, 1.356, 1.288, 1.221, 1.153 ],
[ 1.471, 1.497, 1.519, 1.532, 1.526, 1.474, 1.398, 1.322, 1.246, 1.171 ],
[ 1.624, 1.633, 1.634, 1.623, 1.583, 1.524, 1.461, 1.399, 1.338, 1.276 ],
[ 1.753, 1.744, 1.721, 1.677, 1.626, 1.572, 1.518, 1.464, 1.410, 1.356 ],
[ 1.867, 1.836, 1.793, 1.750, 1.701, 1.653, 1.604, 1.555, 1.506, 1.457 ],
[ 1.940, 1.897, 1.845, 1.792, 1.740, 1.687, 1.635, 1.583, 1.531, 1.478 ],
]
# =======================================================================



# =======================================================================
#                           PR 1.8 LPC map
# =======================================================================
# 11 speeds
# 12 index (stall margins)
LPC.WcTable.x = [ 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20, 1.25 ]
LPC.WcTable.y = [ 1.00, 1.20, 1.40, 1.60, 1.80, 2.00, 2.20, 2.40, 2.60, 2.80, 3.00, 3.20 ] 
LPC.WcTable.data = [
[  38.07,  42.93,  47.75,  52.50,  57.18,  61.79,  66.33,  70.79,  75.15,  76.56,  76.56,  76.56 ],
[  54.03,  60.03,  65.92,  71.68,  77.30,  82.78,  88.10,  93.26,  98.25, 101.05, 101.05, 101.05 ],
[  70.32,  77.51,  84.49,  91.24,  97.74, 103.98, 109.94, 115.62, 121.01, 124.64, 124.64, 124.64 ],
[  87.48,  95.68, 103.53, 111.01, 118.09, 124.75, 130.99, 136.80, 142.16, 146.23, 146.23, 146.23 ],
[ 105.85, 114.80, 123.22, 131.09, 138.39, 145.10, 151.21, 156.72, 161.63, 165.73, 165.73, 165.73 ],
[ 125.11, 134.60, 143.35, 151.34, 158.55, 164.97, 170.61, 175.46, 179.55, 182.89, 183.07, 183.07 ],
[ 144.49, 154.57, 163.62, 171.63, 178.59, 184.51, 189.40, 193.30, 196.24, 198.27, 198.41, 198.41 ],
[ 165.91, 176.22, 185.18, 192.79, 199.08, 204.06, 207.79, 210.34, 211.76, 212.15, 212.15, 212.15 ],
[ 188.56, 198.35, 206.68, 213.57, 219.06, 223.19, 226.03, 227.66, 228.16, 228.16, 228.16, 228.16 ],
[ 214.14, 222.19, 228.90, 234.29, 238.42, 241.33, 243.10, 243.79, 243.81, 243.81, 243.81, 243.81 ],
[ 227.85, 234.58, 240.11, 244.50, 247.78, 250.00, 251.22, 251.52, 251.52, 251.52, 251.52, 251.52 ],
]

LPC.effTable.x = [ 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20, 1.25 ]
LPC.effTable.y = [ 1.00, 1.20, 1.40, 1.60, 1.80, 2.00, 2.20, 2.40, 2.60, 2.80, 3.00, 3.20 ] 
LPC.effTable.data = [
[ 0.7256, 0.7656, 0.7978, 0.8195, 0.8274, 0.8164, 0.7494, 0.5651, 0.1931, 0.0000, 0.0000, 0.0000 ],
[ 0.7474, 0.7848, 0.8147, 0.8351, 0.8430, 0.8339, 0.7757, 0.6161, 0.3003, 0.0000, 0.0000, 0.0000 ],
[ 0.7610, 0.7984, 0.8286, 0.8496, 0.8586, 0.8516, 0.7977, 0.6479, 0.3526, 0.0000, 0.0000, 0.0000 ],
[ 0.7744, 0.8117, 0.8421, 0.8637, 0.8738, 0.8685, 0.8183, 0.6765, 0.3970, 0.0000, 0.0000, 0.0000 ],
[ 0.7872, 0.8240, 0.8542, 0.8759, 0.8866, 0.8827, 0.8360, 0.7028, 0.4407, 0.0000, 0.0000, 0.0000 ],
[ 0.7965, 0.8329, 0.8627, 0.8843, 0.8953, 0.8924, 0.8485, 0.7222, 0.4748, 0.0391, 0.0000, 0.0000 ],
[ 0.7997, 0.8368, 0.8673, 0.8896, 0.9013, 0.8991, 0.8561, 0.7310, 0.4858, 0.0551, 0.0000, 0.0000 ],
[ 0.8034, 0.8405, 0.8712, 0.8937, 0.9058, 0.9042, 0.8628, 0.7420, 0.5068, 0.0979, 0.0000, 0.0000 ],
[ 0.8214, 0.8533, 0.8793, 0.8981, 0.9079, 0.9062, 0.8724, 0.7766, 0.5961, 0.2955, 0.0000, 0.0000 ],
[ 0.8425, 0.8663, 0.8853, 0.8985, 0.9047, 0.9025, 0.8778, 0.8117, 0.6929, 0.5052, 0.2255, 0.0000 ],
[ 0.8540, 0.8731, 0.8880, 0.8981, 0.9024, 0.9000, 0.8800, 0.8286, 0.7386, 0.6003, 0.4004, 0.1206 ],
]

LPC.PRtable.x = [ 0.30, 0.40, 0.50, 0.60, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20, 1.25 ]
LPC.PRtable.y = [ 1.00, 1.20, 1.40, 1.60, 1.80, 2.00, 2.20, 2.40, 2.60, 2.80, 3.00, 3.20 ] 
LPC.PRtable.data = [
[ 1.0423, 1.0412, 1.0393, 1.0367, 1.0333, 1.0292, 1.0234, 1.0151, 1.0043, 1.0000, 1.0000, 1.0000 ],
[ 1.0760, 1.0738, 1.0704, 1.0658, 1.0600, 1.0530, 1.0434, 1.0297, 1.0122, 1.0000, 1.0000, 1.0000 ],
[ 1.1215, 1.1180, 1.1127, 1.1055, 1.0965, 1.0856, 1.0707, 1.0497, 1.0228, 1.0000, 1.0000, 1.0000 ],
[ 1.1789, 1.1738, 1.1660, 1.1555, 1.1423, 1.1266, 1.1052, 1.0753, 1.0374, 1.0000, 1.0000, 1.0000 ],
[ 1.2494, 1.2422, 1.2312, 1.2167, 1.1986, 1.1771, 1.1481, 1.1078, 1.0572, 1.0000, 1.0000, 1.0000 ],
[ 1.3353, 1.3253, 1.3105, 1.2910, 1.2669, 1.2384, 1.2002, 1.1476, 1.0822, 1.0056, 1.0000, 1.0000 ],
[ 1.4411, 1.4282, 1.4088, 1.3830, 1.3512, 1.3136, 1.2632, 1.1942, 1.1088, 1.0101, 1.0000, 1.0000 ],
[ 1.5724, 1.5561, 1.5313, 1.4982, 1.4572, 1.4088, 1.3440, 1.2556, 1.1472, 1.0233, 1.0000, 1.0000 ],
[ 1.7323, 1.7101, 1.6785, 1.6379, 1.5888, 1.5318, 1.4572, 1.3572, 1.2358, 1.0982, 1.0000, 1.0000 ],
[ 1.9360, 1.9056, 1.8662, 1.8184, 1.7625, 1.6991, 1.6190, 1.5142, 1.3887, 1.2471, 1.0944, 1.0000 ],
[ 2.0507, 2.0158, 1.9729, 1.9223, 1.8645, 1.8000, 1.7201, 1.6176, 1.4958, 1.3584, 1.2098, 1.0546 ],
]


# =======================================================================
#                           PR 23 HPC map
# =======================================================================
# 13 speeds
# 11 index (stall margins)
HPC.WcTable.x = [ 0.50, 0.60, 0.70, 0.75, 0.80, 0.85, 0.90, 0.925, 0.95, 0.975, 1.00, 1.025, 1.05 ]
HPC.WcTable.y = [ 1.00, 1.20, 1.40, 1.60, 1.80, 2.00, 2.20, 2.40, 2.60, 2.80, 3.00 ] 
HPC.WcTable.data = [
[  22.7411,  24.0487,  25.1548,  26.0615,  26.7738,  27.2992,  27.6470,  27.8286,  27.8634,  27.8634,  27.8634 ],
[  31.7548,  33.1181,  34.2670,  35.2054,  35.9397,  36.4783,  36.8308,  37.0085,  37.0362,  37.0362,  37.0362 ],
[  46.1066,  47.4088,  48.5066,  49.4046,  50.1096,  50.6291,  50.9717,  51.1469,  51.1757,  51.1757,  51.1757 ],
[  56.7268,  58.0480,  59.1608,  60.0704,  60.7837,  61.3084,  61.6527,  61.8260,  61.8517,  61.8517,  61.8517 ],
[  70.1448,  71.5163,  72.6688,  73.6088,  74.3429,  74.8795,  75.2269,  75.3943,  75.4134,  75.4134,  75.4134 ],
[  89.3764,  90.9746,  92.3098,  93.3900,  94.2232,  94.8199,  95.1897,  95.3442,  95.3504,  95.3504,  95.3504 ],
[ 118.0620, 120.1207, 121.8253, 123.1867, 124.2166, 124.9292, 125.3385, 125.4609, 125.4609, 125.4609, 125.4609 ],
[ 138.5093, 140.8966, 142.8639, 144.4238, 145.5916, 146.3836, 146.8174, 146.9192, 146.9192, 146.9192, 146.9192 ],
[ 160.6243, 162.5676, 164.1805, 165.4722, 166.4536, 167.1370, 167.5334, 167.6563, 167.6563, 167.6563, 167.6563 ],
[ 181.7993, 183.4993, 184.9150, 186.0545, 186.9260, 187.5389, 187.9029, 188.0273, 188.0271, 188.0271, 188.0271 ],
[ 202.6315, 203.5858, 204.3958, 205.0661, 205.5998, 206.0000, 206.2702, 206.4145, 206.4418, 206.4418, 206.4418 ],
[ 209.9986, 210.5917, 211.1029, 211.5321, 211.8825, 212.1554, 212.3516, 212.4735, 212.5220, 212.5227, 212.5227 ],
[ 216.6847, 217.0279, 217.3287, 217.5860, 217.8015, 217.9767, 218.1106, 218.2041, 218.2586, 218.2739, 218.2739 ],
]

HPC.effTable.x = [ 0.50, 0.60, 0.70, 0.75, 0.80, 0.85, 0.90, 0.925, 0.95, 0.975, 1.00, 1.025, 1.05 ]
HPC.effTable.y = [ 1.00, 1.20, 1.40, 1.60, 1.80, 2.00, 2.20, 2.40, 2.60, 2.80, 3.00 ] 
HPC.effTable.data = [
[ 0.6753, 0.6913, 0.7016, 0.7050, 0.7004, 0.6864, 0.6570, 0.6044, 0.5236, 0.4075, 0.2467 ],
[ 0.6953, 0.7094, 0.7184, 0.7214, 0.7176, 0.7058, 0.6812, 0.6378, 0.5717, 0.4783, 0.3512 ],
[ 0.7248, 0.7359, 0.7429, 0.7452, 0.7424, 0.7335, 0.7154, 0.6838, 0.6366, 0.5713, 0.4848 ],
[ 0.7427, 0.7533, 0.7600, 0.7627, 0.7606, 0.7533, 0.7379, 0.7108, 0.6703, 0.6147, 0.5414 ],
[ 0.7634, 0.7736, 0.7804, 0.7834, 0.7822, 0.7762, 0.7630, 0.7394, 0.7041, 0.6556, 0.5920 ],
[ 0.7891, 0.8008, 0.8090, 0.8134, 0.8136, 0.8092, 0.7974, 0.7754, 0.7417, 0.6950, 0.6335 ],
[ 0.8139, 0.8280, 0.8385, 0.8449, 0.8469, 0.8439, 0.8333, 0.8117, 0.7779, 0.7303, 0.6671 ],
[ 0.8206, 0.8356, 0.8469, 0.8541, 0.8567, 0.8544, 0.8442, 0.8229, 0.7892, 0.7416, 0.6783 ],
[ 0.8403, 0.8512, 0.8593, 0.8643, 0.8660, 0.8641, 0.8566, 0.8415, 0.8179, 0.7852, 0.7423 ],
[ 0.8408, 0.8492, 0.8552, 0.8588, 0.8597, 0.8578, 0.8516, 0.8394, 0.8209, 0.7954, 0.7624 ],
[ 0.8470, 0.8505, 0.8529, 0.8539, 0.8536, 0.8520, 0.8483, 0.8418, 0.8324, 0.8200, 0.8043 ],
[ 0.8350, 0.8364, 0.8371, 0.8370, 0.8362, 0.8346, 0.8318, 0.8275, 0.8217, 0.8141, 0.8049 ],
[ 0.8202, 0.8203, 0.8201, 0.8195, 0.8185, 0.8171, 0.8152, 0.8124, 0.8088, 0.8045, 0.7992 ],
]

HPC.PRtable.x = [ 0.50, 0.60, 0.70, 0.75, 0.80, 0.85, 0.90, 0.925, 0.95, 0.975, 1.00, 1.025, 1.05 ]
HPC.PRtable.y = [ 1.00, 1.20, 1.40, 1.60, 1.80, 2.00, 2.20, 2.40, 2.60, 2.80, 3.00 ] 
HPC.PRtable.data = [
[  2.4769,  2.4288,  2.3620,  2.2778,  2.1774,  2.0627,  1.9284,  1.7711,  1.5958,  1.4083,  1.2146 ],
[  3.4633,  3.3778,  3.2643,  3.1248,  2.9619,  2.7787,  2.5679,  2.3253,  2.0595,  1.7802,  1.4973 ],
[  5.0821,  4.9375,  4.7554,  4.5391,  4.2923,  4.0194,  3.7106,  3.3602,  2.9800,  2.5826,  2.1813 ],
[  6.3490,  6.1658,  5.9371,  5.6667,  5.3594,  5.0204,  4.6377,  4.2042,  3.7342,  3.2431,  2.7467 ],
[  8.0021,  7.7686,  7.4792,  7.1388,  6.7532,  6.3287,  5.8504,  5.3097,  4.7237,  4.1114,  3.4919 ],
[ 10.4899, 10.1976,  9.8249,  9.3786,  8.8669,  8.2989,  7.6539,  6.9201,  6.1229,  5.2899,  4.4495 ],
[ 14.4564, 14.0970, 13.6074, 12.9977, 12.2808, 11.4715, 10.5377,  9.4621,  8.2878,  7.0614,  5.8306 ],
[ 17.4426, 17.0500, 16.4870, 15.7661, 14.9034, 13.9183, 12.7692, 11.4347,  9.9718,  8.4425,  6.9104 ],
[ 20.7403, 20.2486, 19.6093, 18.8329, 17.9324, 16.9227, 15.7626, 14.4263, 12.9562, 11.3983,  9.8011 ],
[ 23.8298, 23.2601, 22.5536, 21.7200, 20.7705, 19.7178, 18.5212, 17.1524, 15.6466, 14.0424, 12.3810 ],
[ 26.6962, 26.0933, 25.4175, 24.6733, 23.8656, 22.9999, 22.0495, 20.9930, 19.8436, 18.6163, 17.3267 ],
[ 27.6439, 27.0687, 26.4522, 25.7969, 25.1052, 24.3798, 23.6033, 22.7614, 21.8600, 20.9058, 19.9057 ],
[ 28.4663, 27.9667, 27.4460, 26.9054, 26.3460, 25.7690, 25.1640, 24.5225, 23.8472, 23.1399, 22.4038 ],
]


# =======================================================================
#                                 HPT map
# =======================================================================
#  6 speeds
# 20 index (pressure ratio)

HPT.WcTable.x = [ 0.60, 0.70, 0.80, 0.90, 1.00, 1.10 ]
HPT.WcTable.y = [ 3.00, 3.25, 3.50, 3.75, 4.00, 4.25, 4.50, 4.75, 5.00, 5.25, 5.50, 5.75, 6.00, 6.25, 6.50, 6.75, 7.00, 7.25, 7.50, 8.00 ]
HPT.WcTable.data = [
[ 30.446, 30.533, 30.568, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572, 30.572 ],
[ 30.299, 30.413, 30.480, 30.516, 30.529, 30.530, 30.530, 30.530, 30.530, 30.530, 30.530, 30.530, 30.530, 30.530, 30.530, 30.530, 30.530, 30.530, 30.530, 30.530 ],
[ 30.120, 30.239, 30.317, 30.368, 30.398, 30.415, 30.421, 30.421, 30.421, 30.421, 30.421, 30.421, 30.421, 30.421, 30.421, 30.421, 30.421, 30.421, 30.421, 30.421 ],
[ 30.014, 30.124, 30.201, 30.253, 30.288, 30.311, 30.325, 30.333, 30.337, 30.337, 30.337, 30.337, 30.337, 30.337, 30.337, 30.337, 30.337, 30.337, 30.337, 30.337 ],
[ 29.856, 29.948, 30.013, 30.059, 30.091, 30.113, 30.128, 30.139, 30.145, 30.149, 30.150, 30.150, 30.150, 30.150, 30.150, 30.150, 30.150, 30.150, 30.150, 30.150 ],
[ 29.799, 29.870, 29.920, 29.955, 29.979, 29.997, 30.009, 30.017, 30.023, 30.026, 30.028, 30.029, 30.029, 30.029, 30.029, 30.029, 30.029, 30.029, 30.029, 30.029 ],
]

HPT.effTable.x = [ 0.60, 0.70, 0.80, 0.90, 1.00, 1.10 ]
HPT.effTable.y = [ 3.00, 3.25, 3.50, 3.75, 4.00, 4.25, 4.50, 4.75, 5.00, 5.25, 5.50, 5.75, 6.00, 6.25, 6.50, 6.75, 7.00, 7.25, 7.50, 8.00 ]
HPT.effTable.data = [
[ 0.8460, 0.8405, 0.8349, 0.8296, 0.8249, 0.8206, 0.8165, 0.8127, 0.8092, 0.8060, 0.8030, 0.8002, 0.7976, 0.7953, 0.7931, 0.7911, 0.7892, 0.7875, 0.7858, 0.7826 ],
[ 0.8879, 0.8842, 0.8804, 0.8769, 0.8735, 0.8701, 0.8670, 0.8640, 0.8614, 0.8590, 0.8568, 0.8548, 0.8529, 0.8511, 0.8495, 0.8479, 0.8460, 0.8436, 0.8414, 0.8373 ],
[ 0.9125, 0.9111, 0.9096, 0.9078, 0.9055, 0.9034, 0.9014, 0.8995, 0.8979, 0.8964, 0.8950, 0.8936, 0.8924, 0.8903, 0.8877, 0.8853, 0.8830, 0.8808, 0.8787, 0.8749 ],
[ 0.9228, 0.9242, 0.9250, 0.9247, 0.9240, 0.9232, 0.9224, 0.9217, 0.9210, 0.9203, 0.9197, 0.9188, 0.9162, 0.9137, 0.9113, 0.9091, 0.9070, 0.9050, 0.9031, 0.8980 ],
[ 0.9215, 0.9258, 0.9289, 0.9304, 0.9313, 0.9319, 0.9323, 0.9326, 0.9328, 0.9329, 0.9330, 0.9311, 0.9288, 0.9266, 0.9245, 0.9225, 0.9206, 0.9188, 0.9161, 0.9107 ],
[ 0.9106, 0.9176, 0.9232, 0.9267, 0.9292, 0.9312, 0.9327, 0.9340, 0.9351, 0.9361, 0.9369, 0.9349, 0.9329, 0.9311, 0.9293, 0.9276, 0.9259, 0.9239, 0.9212, 0.9161 ],
]


# =======================================================================
#                                 LPT map
# =======================================================================
#  7 speeds
# 20 index (pressure ratio)

LPT.WcTable.x = [ 0.60, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20 ]
LPT.WcTable.y = [ 3.00, 3.25, 3.50, 3.75, 4.00, 4.25, 4.50, 4.75, 5.00, 5.25, 5.50, 5.75, 6.00, 6.25, 6.50, 6.75, 7.00, 7.25, 7.50, 8.00 ]
LPT.WcTable.data = [
[ 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812, 153.812 ],
[ 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511, 153.511 ],
[ 152.799, 152.982, 153.052, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061, 153.061 ],
[ 150.995, 151.316, 151.518, 151.647, 151.729, 151.781, 151.814, 151.834, 151.846, 151.852, 151.856, 151.858, 151.859, 151.859, 151.859, 151.859, 151.859, 151.859, 151.859, 151.859 ],
[ 148.751, 149.107, 149.349, 149.517, 149.635, 149.719, 149.779, 149.822, 149.852, 149.872, 149.885, 149.894, 149.898, 149.899, 149.899, 149.899, 149.899, 149.899, 149.899, 149.899 ],
[ 145.352, 145.680, 145.905, 146.061, 146.169, 146.244, 146.293, 146.324, 146.339, 146.344, 146.344, 146.344, 146.344, 146.344, 146.344, 146.344, 146.344, 146.344, 146.344, 146.344 ],
[ 140.863, 141.131, 141.310, 141.428, 141.503, 141.547, 141.567, 141.569, 141.569, 141.569, 141.569, 141.569, 141.569, 141.569, 141.569, 141.569, 141.569, 141.569, 141.569, 141.569 ],
]

LPT.effTable.x = [ 0.60, 0.70, 0.80, 0.90, 1.00, 1.10, 1.20 ]
LPT.effTable.y = [ 3.00, 3.25, 3.50, 3.75, 4.00, 4.25, 4.50, 4.75, 5.00, 5.25, 5.50, 5.75, 6.00, 6.25, 6.50, 6.75, 7.00, 7.25, 7.50, 8.00 ]
LPT.effTable.data = [
[  0.8388,  0.8309,  0.8234,  0.8159,  0.8091,  0.8030,  0.7975,  0.7924,  0.7876,  0.7832,  0.7791,  0.7753,  0.7717,  0.7684,  0.7652,  0.7623,  0.7595,  0.7568,  0.7542,  0.7495 ],
[  0.8878,  0.8813,  0.8745,  0.8685,  0.8629,  0.8577,  0.8528,  0.8484,  0.8443,  0.8404,  0.8368,  0.8334,  0.8302,  0.8272,  0.8242,  0.8210,  0.8179,  0.8150,  0.8122,  0.8071 ],
[  0.9201,  0.9152,  0.9105,  0.9061,  0.9018,  0.8978,  0.8940,  0.8905,  0.8872,  0.8840,  0.8810,  0.8776,  0.8741,  0.8707,  0.8676,  0.8646,  0.8618,  0.8590,  0.8565,  0.8516 ],
[  0.9381,  0.9360,  0.9336,  0.9310,  0.9283,  0.9257,  0.9231,  0.9206,  0.9182,  0.9153,  0.9119,  0.9087,  0.9056,  0.9027,  0.8999,  0.8973,  0.8948,  0.8924,  0.8901,  0.8858 ],
[  0.9447,  0.9455,  0.9456,  0.9450,  0.9440,  0.9429,  0.9417,  0.9404,  0.9383,  0.9355,  0.9327,  0.9301,  0.9276,  0.9252,  0.9229,  0.9207,  0.9186,  0.9165,  0.9146,  0.9099 ],
[  0.9415,  0.9454,  0.9479,  0.9495,  0.9504,  0.9510,  0.9512,  0.9511,  0.9492,  0.9472,  0.9452,  0.9433,  0.9414,  0.9396,  0.9378,  0.9361,  0.9344,  0.9326,  0.9304,  0.9262 ],
[  0.9295,  0.9366,  0.9419,  0.9458,  0.9487,  0.9509,  0.9526,  0.9538,  0.9528,  0.9517,  0.9505,  0.9493,  0.9481,  0.9468,  0.9456,  0.9444,  0.9432,  0.9413,  0.9395,  0.9360 ],
]

#--------------------------------------
# declare some output
# ------------------------------------- 
estuff=Output( "turbofan", filename="turbofan.out" )
estuff.vars = [ start.alt, start.MN, start.W, Perf.Fn, Perf.Wfuel ]

#--------------------------------------
# run the DESIGN case
# ------------------------------------- 
varsg.check()
varsg.NS.solve()
varsg.prettyPrint.print()


case = 0

def runThrottleHook( MNset, altitude ):

   global case
   case = case + 1
   fan.NcDem.set( 1. )
   start.MN.set( MNset )
   start.alt.set( altitude )
   varsg.NS.saveInds()
   varsg.NS.run()
   varsg.prettyPrint.print()
   print( start.MN, start.alt, burner.FAR )
   FnetMax = RealT( Perf )
   FnetMax.set( Perf.Fn )
 
   case = case + 1
   varsg.NS.saveInds()
   factor = .9   
   start.Fdem.set( start.Fnet*factor )
   
   fan.dep_NmechC.active=False 
   burner.con_1.on=False 
   burner.con_1.active = False

   burner.ind_FAR.active = False
   varsg.check()

   while( factor > .2 and varsg.NS.converged == True ):
      start.Fdem.set( FnetMax*factor )
      burner.FAR.set( burner.FAR - .0025 )
      varsg.NS.run()
      print( start.MN, start.alt, burner.FAR )	  
      varsg.prettyPrint.print()
      factor = ( Perf.Fn )/FnetMax
      case = case + 1

   fan.dep_NmechC.active=True 
   burner.con_1.on=True 
   burner.ind_FAR.active = True
   varsg.check()
   fan.NcDem.set( 1.0 ) 
   varsg.NS.restoreInds()
   varsg.NS.run()
   case = case + 1



def printStations():
   print( 'CASE # ', case )
   print( '---', start.MN, start.alt, '----------------------------------------------------' )
   print( 'fan Nc =', fan.NcMap, '     burner temperature =', burner.FNo.Tt, '   ambient Ps =', inlet.FNi.Ps )
   print( fan.Rline )
   print( 'inlet in', inlet.FNi.W, inlet.FNi.Pt, inlet.FNi.Tt, inlet.FNi.Ps, inlet.FNi.Ts, inlet.FNi.rhos, inlet.FNi.MN )
   print( 'inlet out', inlet.FNo.W, inlet.FNo.Pt, inlet.FNo.Tt, inlet.FNo.Ps, inlet.FNo.Ts, inlet.FNo.rhos, inlet.FNo.MN )
   print( 'fan out', fan.FNo.W, fan.FNo.Pt, fan.FNo.Tt, fan.FNo.Ps, fan.FNo.Ts, fan.FNo.rhos, fan.FNo.MN )
   print( 'duct2 out', LPC.FNi.W, LPC.FNi.Pt, LPC.FNi.Tt, LPC.FNi.Ps, LPC.FNi.Ts, LPC.FNi.rhos, LPC.FNi.MN )
   print( 'LPC out', LPC.FNo.W, LPC.FNo.Pt, LPC.FNo.Tt, LPC.FNo.Ps, LPC.FNo.Ts, LPC.FNo.rhos, LPC.FNo.MN )
   print( 'duct25 out', HPC.FNi.W, HPC.FNi.Pt, HPC.FNi.Tt, HPC.FNi.Ps, HPC.FNi.Ts, HPC.FNi.rhos, HPC.FNi.MN )
   print( 'HPC out', HPC.FNo.W, HPC.FNo.Pt, HPC.FNo.Tt, HPC.FNo.Ps, HPC.FNo.Ts, HPC.FNo.rhos, HPC.FNo.MN )
   print( 'burner out', burner.FNo.W, burner.FNo.Pt, burner.FNo.Tt, burner.FNo.Ps, burner.FNo.Ts, burner.FNo.rhos, burner.FNo.MN )
   print( 'HPT out', HPT.FNo.W, HPT.FNo.Pt, HPT.FNo.Tt, HPT.FNo.Ps, HPT.FNo.Ts, HPT.FNo.rhos, HPT.FNo.MN )
   print( 'HPT', HPT.eff )
   print( 'duct45 out', duct45.FNo.W, duct45.FNo.Pt, duct45.FNo.Tt, duct45.FNo.Ps, duct45.FNo.Ts, duct45.FNo.rhos, duct45.FNo.MN )
   print( 'LPT out', LPT.FNo.W, LPT.FNo.Pt, LPT.FNo.Tt, LPT.FNo.Ps, LPT.FNo.Ts, LPT.FNo.rhos, LPT.FNo.MN )
   print( 'LPT ', LPT.eff )
   print( 'duct5 out', duct5.FNo.W, duct5.FNo.Pt, duct5.FNo.Tt, duct5.FNo.Ps, duct5.FNo.Ts, duct5.FNo.rhos, duct5.FNo.MN )
   print( 'core nozzle out', priNozzle.FNo.W, priNozzle.FNo.Pt, priNozzle.FNo.Tt, priNozzle.FNo.Ps, priNozzle.FNo.Ts, priNozzle.FNo.rhos, priNozzle.FNo.MN )
   print( 'duct 17 out', fanNozzle.FNi.W, fanNozzle.FNi.Pt, fanNozzle.FNi.Tt )
   print( 'bypass nozzle out', fanNozzle.FNo.W, fanNozzle.FNo.Pt, fanNozzle.FNo.Tt, fanNozzle.FNo.Ps, fanNozzle.FNo.Ts, fanNozzle.FNo.rhos, fanNozzle.FNo.MN )
   print( ' ' )
   print( priNozzle.FNo.V / fanNozzle.FNo.V )
   print( 'HP power balance: ', HPC.MP.hp, '=', HPT.MP.hp, ' + ', HPshaft.HPX, ' HP extraction'  )
   print( 'LP power balance: ', fan.MP.hp , '+', LPC.MP.hp, '=', fan.MP.hp+LPC.MP.hp, LPT.MP.hp )
   print( ' ' )

varsg.prettyPrint.print()

# this is setting the code from DESIGN (sizing) mode to OFF-DESIGN mode
varsg.set("size", False )
fanNozzle.ind_Area = Independent( fanNozzle, indname="Anoz", perturb=.05, scale=100, perturb_type="Relative", active=True )        
fan.RlineLimit = RealT( fan, v=2.0 )
fan.dep_Rline = Dependent( fan, d1name="Rline", d2name="RlineLimit", active=True )

varsg.check()
varsg.NS.run()
varsg.prettyPrint.print()
#varsg.scottPrint.print()
print( dir() )
quit()

fan.NcDem = RealT( fan )
fan.NcDem.set( 1. )
burner.Tmax = RealT( burner )
burner.Tmax.set( 3360. )
start.Fdem = RealT( start )

start.Fnet = RealT( start )
burner.ind_FAR = Independent( burner, indname="FAR", perturb=.05, perturb_type="Relative", active=True, desc="Varies R-line" )        
fan.dep_NmechC = Dependent( fan, d1name="NcMap", d2name="NcDem", active=True, desc="Handles weight flow error" )
burner.con_1 = Constraint( burner, d1name="burner.Tmax", d2name="FNo.Tt", depname="fan.dep_NmechC", on=True )
#start.dep_Fnet = Dependent( fan, d1name="start.Fnet", d2name="start.Fdem", active=True, desc="Handles weight flow error" )

varsg.check()

runThrottleHook( 0.90, 45000. );
runThrottleHook( 0.85, 45000. );
runThrottleHook( 0.80, 45000. );
runThrottleHook( 0.75, 45000. );
runThrottleHook( 0.70, 45000. );

runThrottleHook( 0.60, 40000. );
runThrottleHook( 0.70, 40000. );
runThrottleHook( 0.75, 40000. );
runThrottleHook( 0.80, 40000. );
runThrottleHook( 0.85, 40000. );
runThrottleHook( 0.90, 40000. );

runThrottleHook( 0.90, 36089. );
runThrottleHook( 0.85, 36089. );
runThrottleHook( 0.80, 36089. );
runThrottleHook( 0.75, 36089. );
runThrottleHook( 0.70, 36089. );
runThrottleHook( 0.60, 36089. );

runThrottleHook( 0.50, 30000. );
runThrottleHook( 0.60, 30000. );
runThrottleHook( 0.70, 30000. );
runThrottleHook( 0.75, 30000. );
runThrottleHook( 0.80, 30000. );
runThrottleHook( 0.85, 30000. );

runThrottleHook( 0.80, 25000. );
runThrottleHook( 0.75, 25000. );
runThrottleHook( 0.70, 25000. );
runThrottleHook( 0.60, 25000. );
runThrottleHook( 0.50, 25000. );

runThrottleHook( 0.40, 20000. );
runThrottleHook( 0.50, 20000. );
runThrottleHook( 0.60, 20000. );
runThrottleHook( 0.70, 20000. );
runThrottleHook( 0.75, 20000. );

runThrottleHook( 0.70, 15000. );
runThrottleHook( 0.60, 15000. );
runThrottleHook( 0.50, 15000. );
runThrottleHook( 0.40, 15000. );
runThrottleHook( 0.30, 15000. );

runThrottleHook( 0.60, 10000. );
runThrottleHook( 0.50, 10000. );
runThrottleHook( 0.40, 10000. );


runThrottleHook( 0.00,  5000. );
runThrottleHook( 0.10,  5000. );
runThrottleHook( 0.20,  5000. );
runThrottleHook( 0.25,  5000. );
runThrottleHook( 0.30,  5000. );
runThrottleHook( 0.40,  5000. );
runThrottleHook( 0.50,  5000. );

runThrottleHook( 0.50,  2000. );
runThrottleHook( 0.40,  2000. );
runThrottleHook( 0.30,  2000. );
runThrottleHook( 0.25,  2000. );
runThrottleHook( 0.20,  2000. );
runThrottleHook( 0.10,  2000. );
runThrottleHook( 0.00,  2000. );

runThrottleHook( 0.50,     0. );
runThrottleHook( 0.40,     0. );
runThrottleHook( 0.30,     0. );
runThrottleHook( 0.25,     0. );
runThrottleHook( 0.20,     0. );
runThrottleHook( 0.10,     0. );
runThrottleHook( 0.00,     0. );

print( time.time() - start_time, case, ( time.time() - start_time )/ case )