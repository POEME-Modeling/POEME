...
class Turbine(Element):
    ...
    def __init__(t, name):
        ...
        # tables
        t.effTable = Table2d(
            t, desc="Table of efficiency versus corrected speed and pressure ratio"
        )
        t.WcTable = Table2d(
            t,
            units="lbm/sec",
            desc="Table of corrected weight flow versus corrected speed and pressure ratio",
        )
        ...
        t.dep_TW = Dependent(
            t, d1name="Wc", d2name="WcMap", active=False, desc="Handles flow error"
        )
        ...
        t.size = BooleanT(
            t, v=True, desc="determines if the turbine is in sizing mode or not"
        )
        t.initialList()
    ...
    def calc(t):
        ...
        t.MP.setHP(-1 * (t.FN42.ht - t.FN41.ht) * t.FN41.W * 3600.0 / 2545.0)
    ...
    def pretty(t):
        print(
            f"{"Turbine"[:10]:12s}{t.name1[:10]:12s}{("PR:"+str(t.PR))[:10]:12s}{("eff:"+str(t.eff))[:10]:12s}{("PRmap:"+str(t.PRmap))[:10]:12s}{("NcMap:"+str(t.NcMap))[:10]:12s}",
            file=g.pretty,
        )
