...
class Nozzle(Element):
    ...
    def __init__(n, name):
        ...
        # variables
        n.Cfg = RealT(n, v=1.0, units="non", desc="Coefficient of gross thrust")
        ...
        # dependents
        n.dep_NA = Dependent(
            n, d1name="Anoz", d2name="FNo.A", active=False, desc="Nozzle area error"
        )
        n.size = BooleanT(
            n, v=True, desc="determines if nozzle is in sizing mode or not"
        )
    ...
    def calc(n):
        ...
        n.Fg += n.Cfg * (
            n.FNo.W * n.FNo.V / 32.17 + n.FNo.A * 144.0 * (n.FNo.Ps - n.PsExh.get())
        )
    ...
    def pretty(n):
        print(
            f"{"Nozzle"[:10]:12s}{n.name1[:10]:12s}{("Fg:"+str(n.Fg))[:10]:12s}",
            file=g.pretty,
        )
