...
class Perf(Element):
    ...
    def pretty(p):
        print(
            f"{"Performance"[:10]:12s}{p.name1[:10]:12s}{("alt:"+str(p.alt))[:10]:12s}{("MN:"+str(p.MN))[:10]:12s}{("Fn:"+str(p.Fn))[:10]:12s}{("SFC:"+str(p.SFC))[:10]:12s}{("Fg:"+str(p.Fg))[:10]:12s}{("Fram:"+str(p.Fram))[:10]:12s}{("Wfuel:"+str(p.Wfuel))[:10]:12s} ",
            file=g.pretty,
        )
