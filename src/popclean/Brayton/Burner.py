...
class Burner(Element):
    ...
    def __init__(b, name):
        ...
        b.eff = RealT(b, v=1.0, units="none", desc="Burner efficiency")
    ...
    def pretty(b):
        print(
            f"{"Burner"[:10]:12s}{b.name1[:10]:12s}{("FAR:"+str(b.FAR))[:10]:12s}{("Tout:"+str(b.Tout))[:10]:12s}{("Wfuel:"+str(b.Wfuel))[:10]:12s}{("dP:"+str(b.dP))[:10]:12s}",
            file=g.pretty,
        )
