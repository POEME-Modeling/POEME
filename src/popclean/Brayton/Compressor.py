...
class Compressor(Element):
    ...
    def __init__(c, name):
        ...
        # tables
        c.effTable = Table2d(
            c, desc="Table of efficiency versus corrected speed and Rline"
        )
        c.PRtable = Table2d(
            c, desc="Table of pressure ratio versus corrected speed and flow"
        )
        c.WcTable = Table2d(
            c,
            units="lbm/sec",
        ...
        c.FNi = FN(c, io="in", desc="Incoming fluid flow")
        c.FNideal = FN(c, desc="Ideal exit conditions")
        c.FNo = FN(c, io="out", desc="Outgoing fluid flow")
        c.FNoBld1 = FN(c, io="out", desc="Bleed flow 1")
        c.FNoBld2 = FN(c, io="out", desc="Bleed flow 2")

        # mechanical connections
        c.MP = MP(c, io="out", desc="Mechanicl port connectio")

        # solver stuff
        c.ind_CR = Independent(
            c,
            indname="Rline",
            perturb=0.05,
            scale=100,
        ...
        c.effScale = RealT(c, units="none", desc="Scalar on efficiency map")
        c.hfract1 = RealT(
            c,
            units="BTU/lbm",
            desc="Enthalpy fraction that determines how down the compressor the bleed is taken",
        )
        c.hfract2 = RealT(
            c,
            units="BTU/lbm",
        ...
        c.Nc = RealT(c, units="RPM/R**.5", desc="Corrected speed")
        c.NcMap = RealT(
            c, units="RPM/R**.5", desc="Corrected speed used to read the map"
        )
        c.NcMapDes = RealT(c, units="RPM/R**.5", desc="Design point corrected speed")
        c.NcScale = RealT(c, units="none", desc="Scalar on corrected speed")
        c.PR = RealT(c, units="none", desc="Pressure ratio")
        c.PRdes = RealT(c, units="none", desc="Design pressure ratio")
        ...
        c.WcScale = RealT(c, units="none", desc="Scalar on corrected flow")
        c.Wfrac1 = RealT(c, units="none", desc="Weight flow fraction for bleed flow 1")
        c.Wfrac2 = RealT(c, units="none", desc="Weight flow fraction for bleed flow 2")

        c.size = BooleanT(
            c, v=True, desc="Determine if the element is in design mode or not"
        )
        c.initialList()
    ...
    def calc(c):
        ...
        c.Nc = c.MP.N / (c.FNi.Tt) ** 0.5
        c.Wc = c.FNi.W * (c.FNi.Tt) ** 0.5 / c.FNi.Pt
        c.Wc = c.FNi.W * (c.FNi.Tt) ** 0.5 / c.FNi.Pt
        ...
        if c.Wfrac1 > 0.0:
            ...
            c.FNoBld1.set_hP(
                c.FNi.ht + c.hfract1 * (c.FNo.ht - c.FNi.ht),
                c.hfract1 * (c.FNo.Pt - c.FNi.Pt),
            )
        if c.Wfrac2 > 0.0:
            c.FNoBld2.setW(c.Wfrac2 * c.FNi.W)
            c.FNoBld2.set_hP(
                c.FNi.ht + c.hfract2 * (c.FNo.ht - c.FNi.ht),
                c.hfract2 * (c.FNo.Pt - c.FNi.Pt),
            )
        ...
        c.MP.setHP(
            -1.0
            * (
                (c.FNo.ht - c.FNi.ht) * c.FNo.W * 3600.0 / 2545.0
                + (c.FNoBld1.ht - c.FNi.ht) * c.FNoBld1.W * 3600.0 / 2545.0
                + (c.FNoBld2.ht - c.FNi.ht) * c.FNoBld2.W * 3600.0 / 2545.0
            )
        )
    ...
    def pretty(c):
        print(
            f"{"Compressor"[:10]:12s}{c.name1[:10]:12s}{("PR:"+str(c.PR))[:10]:12s}{("eff:"+str(c.eff))[:10]:12s}{("Rline:"+str(c.Rline))[:10]:12s}{("NcMap:"+str(c.NcMap))[:10]:12s}",
            file=g.pretty,
        )
