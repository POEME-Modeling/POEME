...
class Newton(Element):
    ...
    def __init__(s, name):
        ...
        s.maxJacobians = RealT(
            s, v=50.0, units="Integer", desc="Maxium number of Jacobians"
        )
        s.numpasses = RealT(s, v=0.0, units="Integer", desc="Number of passes")
        s.tolerance = RealT(s, v=0.0001, units="real", desc="tolernace")
        s.constraints = False
        g.solver = s
        s.type = "NewtonSolver"
        s.time = RealT(s, v=0.0, units="seconds", dessc="Simulation time")
            ...
            while iter < s.maxJacobians.v and s.converged == False:
                ...
                if iter > 1 and len(s.ind_list) > 1 and errSum < errSumLast:
                    ...
                    for d in s.dep_list:
                        if d.active == True:
                            ...
                            errSum = errSum + d.baseError**2.0

                    for st in s.state_list:
                        if st.active == True:
                            st.baseError = st.depError()
                            errSum = errSum + st.baseError**2.0

                    for c in s.con_list:
                        if c.active == True:
                            c.baseError = c.depError()
                            errSum = errSum + c.baseError**2.0
                ...
                while check == 0:
                    ...
                    for i in s.ind_list:
                        ...
                        if i.scale.v != 0:
                            scale = i.scale.v
                        if abs(delx[ic] / scale) > 0.1:
                            if abs(delx[ic]) > abs(0.1 * scale):
                                if iscale > abs(0.1 * scale / delx[ic]):
                                    iscale = abs(0.1 * scale / delx[ic])
                    ...
                    for d in s.dep_list:
                        if d.active == True:
                            errSum = errSum + d.depError() ** 2.0

                    for st in s.state_list:
                        if st.active == True:
                            errSum = errSum + st.depError() ** 2.0
